...
||isset($_POST['demo_submit']) ) {
    ...
    $name_query = "SELECT full_name_mail FROM voterinfo WHERE voter_reg_num = :num;";
...
if (isset($_POST['history_submit'])) {
    ...
    if ($history_result && $prepared_stmt_history->rowCount() > 0) { ?>
        ...
        <!-- Then create a table-like structure. See the project.css how the table is stylized. -->
        <table>
            <!-- Create the first row of the table as table head (thead). -->
            <thead>
                <!-- The top row is the table head with three columns named -->
                <tr>
                    <th>Election ID</th>
                    <th>Voting Method</th>
                    <th>Party</th>
            ...
            <!-- Create the rest of the body of the table -->
            <tbody>
                <!-- For each row saved in the $history_result variable ... -->
    ...
    <?php } else { ?>
        <!-- If the query execution resulted in an error, display the following message -->
        <h3>Sorry, no results found for voter <?php echo $_POST['reg_num']; ?>. </h3>
<?php }
} ?>
